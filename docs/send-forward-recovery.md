...
- Persistence preserves the distinction between temporary leader/quorum/transport unavailability (`ReasonNodeNotMatch`) and permanent errors such as idempotency content conflicts. A timeout or transport failure after a persistent SEND without `ClientMsgNo` is outcome-ambiguous, so it receives no local failure ACK; keyed SENDs may return a retryable result because PR46 provides durable deduplication and canonical message identity.
...
- Worker admission happens before dequeuing, so saturation preserves queued user/channel events. Synchronous forwarding waits pass through a non-blocking gate capped at one quarter of the configured user or channel worker count. Excess SEND batches fail fast with a retryable ACK; fixed-destination lifecycle, write, distribution and webhook batches fall back to the reconnect-aware legacy transport before any v1 admission attempt, because they have no client retry path. The lower transport queue is bounded by count/bytes; failed batches remain charged to backpressure until reconnect or shutdown. Shutdown releases retained memory and records drops. No durable queue/outbox is introduced.
