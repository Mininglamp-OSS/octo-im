...
- Only persistent SENDs with nonempty `client_msg_no`, ACKs and ping/pong frames can be retried after an ambiguous request. Other events stop on ambiguity. An ambiguous non-replayable SEND receives the non-retryable `ReasonSystemError`; definite admission failures and replayable SENDs receive `ReasonNodeNotMatch`. Post-commit distribution failures never emit a second contradictory SENDACK.
