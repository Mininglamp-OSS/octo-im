...
func (ch *channel) limitSubscriberRequests(next func(*wkhttp.Context)) func(*wkhttp.Context) {
	return func(c *wkhttp.Context) {
		if options.G.SubscriberRecovery.Enabled {
			select {
			case ch.recoveryAdmission <- struct{}{}:
				defer func() { <-ch.recoveryAdmission }()
			default:
				c.Header("Retry-After", "1")
				c.JSON(http.StatusTooManyRequests, map[string]any{"status": 429, "msg": "subscriber admission busy; retry with the same operation_id"})
				return
			}
